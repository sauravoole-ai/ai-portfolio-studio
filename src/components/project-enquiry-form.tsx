  ...
  return (
    <form className="surface-feature p-6 sm:p-8 md:p-10" aria-describedby="enquiry-status" onSubmit={submit} noValidate>
      <div className="border-b border-border-subtle pb-7">
        ...
        <h2 className="mt-4 text-2xl tracking-[-0.035em] sm:text-3xl">Connect for work.</h2>
        <p id="enquiry-status" className="mt-4 max-w-2xl text-sm leading-6 text-foreground-soft">Share what you’re considering and the context around it.</p>
      </div>
      ...
      <label className="mt-6 block text-sm font-medium text-foreground">Project type <span className="font-normal text-muted-foreground">— optional</span><select className={inputClass} value={draft.projectType} onChange={(event) => setField("projectType", event.target.value)}><option value="">Select a project type</option>{projectTypes.map((item) => <option key={item}>{item}</option>)}</select></label>
      <label className="mt-6 block text-sm font-medium text-foreground">What are you looking to build?<RequiredMark /><textarea className={`${inputClass} min-h-32 resize-y`} rows={5} maxLength={CONTACT_LIMITS.buildIdea} value={draft.buildIdea} onChange={(event) => setField("buildIdea", event.target.value)} /></label>
      <label className="mt-6 block text-sm font-medium text-foreground">Brief / context<RequiredMark /><textarea className={`${inputClass} min-h-44 resize-y`} rows={7} maxLength={CONTACT_LIMITS.message} value={draft.message} onChange={(event) => setField("message", event.target.value)} /></label>
      ...
      <div className="mt-8 border-t border-border-subtle pt-6">
        ...
        <button type="submit" disabled={state === "submitting"} className="button-primary mt-5">{state === "submitting" ? "Sending…" : "Submit enquiry"}</button>
      </div>
    </form>
  );
}
