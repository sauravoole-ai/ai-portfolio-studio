...
function ProfileForm({ profile, onSaved }: { profile: NonNullable<Awaited<ReturnType<typeof getStudioSiteProfile>>>; onSaved: () => Promise<void> }) {
  ...
  const [homeBridgeText, setHomeBridgeText] = useState(profile.home_bridge_text);
  const [homeWorkBlurb, setHomeWorkBlurb] = useState(profile.home_work_blurb);
  const [homeJournalBlurb, setHomeJournalBlurb] = useState(profile.home_journal_blurb);
  const [footerConnectContext, setFooterConnectContext] = useState(profile.footer_connect_context);
